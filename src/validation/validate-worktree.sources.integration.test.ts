  ...
  it('refuses a marker naming a source the page does not have', async () => {
    ...
    expect(await validateWorktree(BRAIN, root)).toEqual({
      ...
      failure: {
        ...
        reason:
          "projects/toolkit.md: S2 names no entry in this page's sources: list",
      },
    })
  })
  ...
  it('refuses a reordered sources: list on a page carrying markers', async () => {
    ...
    expect(await validateWorktree(BRAIN, root)).toEqual({
      ...
      failure: {
        code: 'CONTENT_VALIDATION_FAILED',
        reason:
          'projects/toolkit.md: sources: was reordered or shortened; on a page carrying claim markers it is append-only, because every marker points at a position',
      },
    })
  })
