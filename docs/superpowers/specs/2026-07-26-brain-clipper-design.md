...
[Chrome, any machine]                       [GitHub]                [Mac with brain]
  click / Cmd+Shift+S
    ...
    service worker: durable job queue -> vetted asset fetch -> Git Data API
      blobs -> tree -> commit -> ref      ->  BusiRocket/brain-clips (private)
...
Repo: `BusiRocket/brain-clipper`. TypeScript, bundled with esbuild, no UI
...
`BusiRocket/brain-clips` alone, authorized per device through the **device
flow**.
...
Nothing about the app needs to be recorded in `~/p/vault`: the client id is
